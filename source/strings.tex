...
such that the \crange{p}{p + n} is valid and
\tcode{this->compare(0, k, p, k) == 0} is \tcode{true} before the call.
The values in the \crange{p + k}{p + n} may be indeterminate\iref{basic.indet}.
\item
...
\item
After evaluating $OP$
there are no indeterminate values in the range \crange{p}{p + r}.
\end{itemize}
