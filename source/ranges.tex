...
namespace std::ranges {
  ...
  public:
    ...
    constexpr auto begin() requires (!(@\libconcept{simple-view}@<Views> && ...)) {
    ...
    constexpr auto end() requires (!(@\libconcept{simple-view}@<Views> && ...)) {
      if constexpr (!@\exposid{zip-is-common}@<Views...>) {
    ...
    constexpr auto end() const requires (@\libconcept{range}@<const Views> && ...) {
      if constexpr (!@\exposid{zip-is-common}@<const Views...>) {
        return sentinel<true>(tuple-transform(ranges::end, @\exposid{views_}@));
      }
      else if constexpr ((random_access_range<const Views> && ...)) {
        return begin() + iter_difference_t<iterator<true>>(size());
      }
      else {
        return iterator<true>(tuple-transform(ranges::end, @\exposid{views_}@));
      }
    }
...
namespace std::ranges {
  ...
  public:
    ...
    friend constexpr auto operator<=>(const @\exposid{iterator}@& x, const @\exposid{iterator}@& y)
      requires @\exposconcept{all-random-access}@<Const, Views...> &&
               (three_way_comparable<iterator_t<@\exposid{maybe-const}@<Const, Views>>> && ...);
...
then iterator_concept denotes \tcode{forward_iterator_tag}.
\item
Otherwise, \tcode{iterator_concept} denotes \tcode{input_iterator_tag}.
\end{itemize}
...
constexpr @\exposid{iterator}@(iterator<!Const> i)
  requires Const &&
    (@\libconcept{convertible_to}@<iterator_t<Views>, iterator_t<@\exposconcept{maybe-const}@<Const, Views>>> && ...);
\end{itemdecl}
...
\begin{codeblock}
(noexcept(ranges::iter_move(declval<iterator_t<@\exposid{maybe-const}@<Const,
                                                            Views>> const&>())) && ...) &&
...
namespace std::ranges {
  template<@\libconcept{copy_constructible}@ F, @\libconcept{input_range}@... Views>
    requires (@\libconcept{view}@<Views> && ...) && (sizeof...(Views) > 0) && is_object_v<F> &&
              @\libconcept{regular_invocable}@<F&, range_reference_t<Views>...> &&
              @\exposid{can-reference}@<invoke_result_t<F&, range_reference_t<Views>...>>
...
namespace std::ranges {
  ...
  class zip_transform_view<F, Views...>::@\exposid{iterator}@ {
    ...
    constexpr @\exposid{iterator}@(@\exposid{Parent}@& parent, @\exposid{ziperator}@<Const> inner); // exposition only
  ...
  public:
    ...
    friend constexpr @\exposid{iterator}@ operator+(difference_type n, const @\exposid{iterator}@& i)
      requires @\libconcept{random_access_range}@<@\exposid{Base}@>;
    friend constexpr @\exposid{iterator}@ operator-(const @\exposid{iterator}@& i, difference_type n)
      requires @\libconcept{random_access_range}@<Base>;
    friend constexpr difference_type operator-(const @\exposid{iterator}@& x, const @\exposid{iterator}@& y)
...
The exception specification is equivalent to
\tcode{\keyword{noexcept}(invoke(*\exposid{parent_}->\exposid{fun_}, *std::get<Is>(\exposid{inner_}.\exposid{current_})...))}.
\end{itemdescr}
...
produces a \libconcept{view} whose $M^\textrm{th}$ element is
a tuple of references to
the $M^\textrm{th}$ through $(M + N - 1)^\textrm{th}$ elements of
the original view.
...
namespace std::ranges {
  ...
  public:
    ...
    constexpr auto begin() requires (!@\libconcept{simple-view}@<V>) {
...
with the corresponding element of i.\exposid{current_} as an xvalue.
\end{itemdescr}
...
\begin{itemdecl}
friend constexpr auto iter_move(const iterator& i) noexcept(@\seebelow@);
\end{itemdecl}
...
The exception specification is equivalent to
\begin{codeblock}
noexcept(ranges::iter_move(declval<iterator_t<@\exposid{Base}@> const&>())) &&
...
whose $M^\textrm{th}$ element is the result of applying the invocable object
to the $M^\textrm{th}$ through $(M + N - 1)^\textrm{th}$ elements
...
namespace std::ranges {
  template<@\libconcept{forward_range}@ V, @\libconcept{copy_constructible}@ F, size_t N>
   requires @\libconcept{view}@<V> && (N > 0) && is_object_v<F> &&
            ...
            @\exposid{can-reference}@<invoke_result_t<F&, @\exposid{REPEAT}@(range_reference_t<V>, N)...>>
