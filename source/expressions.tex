...
void f() {
  ...
  [=](decltype((x)) y){};       // error: \tcode{x} refers to local entity but is in lambda's
  ...
  [=]{
      ...
      [x=1](decltype((x)) z){}; // error: \tcode{x} refers to \grammarterm{init-capture} but is in lambda's
                                // \grammarterm{parameter-declaration-clause}
  };
}
\end{codeblock}
...
\pnum
An \grammarterm{init-capture} inhabits the functino parameter scope of
